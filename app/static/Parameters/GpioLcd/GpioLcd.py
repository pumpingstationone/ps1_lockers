...
class _GpioLcd(LcdApi):
    ...
    def __init__(self, rs_pin, enable_pin, d0_pin=None, d1_pin=None,
        ...
        time.sleep_us(20) # Allow LCD time to powerup
        # Send reset 3 times
        self.hal_write_init_nibble(self.LCD_FUNCTION_RESET)
        time.sleep_us(5)    # need to delay at least 4.1 msec
        self.hal_write_init_nibble(self.LCD_FUNCTION_RESET)
        time.sleep_us(1)
        self.hal_write_init_nibble(self.LCD_FUNCTION_RESET)
        time.sleep_us(1)
    ...
    def hal_pulse_enable(self):
        ...
        self.enable_pin.value(0)
        time.sleep_us(300)     # Commands need > 37us to settle
...
class GpioLcd(Parameter):
    ...
    def __init__(self, *,
        ...
        **k):
        super().__init__(**k)
        self.clear = make_var(clear)
        self.initial_state = make_var(initial_state)
        self.num_columns = num_columns
        self.lcd = _GpioLcd(rs_pin=rs_pin,
    ...
    def update(self):
        super().update()
        self.clear.add_hot(self.do_clear)
        if isinstance(self.initial_state, Stater):
            self.state: dict[str, str] = {'line1': "", "line2": ""}
            self.lcd.clear()
        else:
            self.state: dict[str, str] = self.initial_state.state
            self.write()
    ...
    def __call__(self, state: dict):
        ...
        for k, v in state.items():
            if k not in self.state:
                print('raise some error in the future')
                return
            if len(v) > self.num_columns:
                v = v[:self.num_columns]
            self.state[k] = v
    ...
    def do_clear(self, arg: bool):
        if arg == True:
            for l in self.state.values():
                l = ""
            self.lcd.clear()
        
        
    def write(self):
        for line, string in enumerate(self.state.values()):
            self.lcd.move_to(0,line)
            self.lcd.putstr(f"{string:<{self.num_columns}}") # padd the right to make sure it clears out chars that may be there
