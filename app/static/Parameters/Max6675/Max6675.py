...
class MAX6675:
    ...
    def __init__(self, sck_pin, cs_pin, so_pin):
        ...
        """
        # Thermocouple
        self._sck = machine.Pin(sck_pin, machine.Pin.OUT)
        self._sck.low()

        self._cs = machine.Pin(cs_pin, machine.Pin.OUT)
        self._cs.high()

        self._so = machine.Pin(so_pin, machine.Pin.IN)
        self._so.low()
...
class Max6675(Parameter):
    struct = 'f'
    
    def __init__(self, *, celsius, delay, sck_pin, cs_pin, so_pin, **k):
        super().__init__(**k)
        self.sensor = MAX6675(sck_pin=sck_pin, cs_pin=cs_pin, so_pin=so_pin)
        self.celsius = make_var(celsius)
