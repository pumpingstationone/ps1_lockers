...
class HX711Core:
    ...
    def tare(self):
        self.zero = self.read(corrected=False)
        print(f'taring, val = {self.zero}')
    ...
    def read(self, corrected=True):
        val = self._read()
        if corrected:
            return val - self.zero
        else:
            return val
    ...
    def _read(self, raw=False):
        ...
        """
        if not self.is_ready():
            self._wait()  # TODO: make this async fucntion
...
class Hx711(Parameter):
    struct = 'f'
    channels = {
        "CHANNEL_A_128": HX711Core.CHANNEL_A_128,
        "CHANNEL_A_64": HX711Core.CHANNEL_A_64,
        "CHANNEL_B_32": HX711Core.CHANNEL_B_32
    }
    def __init__(self, *, tare, d_out, pd_sck, channel, **k):
        super().__init__(**k)
        self.tare = make_var(tare)
        
        _channel = self.channels[channel]
        self.scale = HX711Core(d_out=d_out, pd_sck=pd_sck, channel=_channel)
    ...
    def update(self):
        super().update()
        self.tare.add_hot(self.do_tare)
        self.scale.tare()
        sleep_us(500)
        self.state = self.scale.read()
        self.ring[0] = self.state
        self.ring = self.ring*3
        
    
    def do_tare(self, state):
        if state == True:
            self.scale.tare()
    
    async def chk(self) -> None:
        while True:    
            self.ring[self.index] = self.scale.read()
            self.index += 1
            if self.index == len(self.ring):
                self.index = 0
            state = sum(self.ring)//len(self.ring)
            
            if abs(self.state - state) > 200:
                self.state = state
                self.send()
