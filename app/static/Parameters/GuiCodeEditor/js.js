...
var GuiCodeEditor = {
  ...
  send: function (pid) {
    const text = GuiCodeEditor.assets[`${pid}_editor`].getValue();
    ...
    document.getElementById(`${param.pid}_file_input`).addEventListener('change', function (event) {
      ...
      if (fileInput.files.length > 0) {
        ...
        reader.onload = function (e) {
          ...
          GuiCodeEditor.assets[`${param.pid}_editor`].setValue(fileContent);
        };
  ...
  save_file: function (pid) {
    ...
    console.log(GuiCodeEditor.assets[`${pid}_editor`].getValue())
    const fileName = prompt('Enter a filename: ', "filename.evzr");

    if (fileName) {
      const fileContent = GuiCodeEditor.assets[`${pid}_editor`].getValue();
